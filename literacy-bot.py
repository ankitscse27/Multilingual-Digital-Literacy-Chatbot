...
LANG_DATA = {
    'en': {
        ...
        'creator': "This chatbot was made by Anup Yadav.",
    ...
    'hi': {
        ...
        'creator': "इस चैटबॉट को अनूप यादव ने बनाया है।",
    ...
    'hing': {
        ...
        'creator': "Is chatbot ko Anup Yadav ne banaya hai.",
    ...
    'awa': {
        ...
        'creator': "इ चैटबॉट के अनूप यादव बनउले हइन।",
    ...
    'guj': {
        ...
        'creator': "આ ચેટબોટ અનુપ યાદવે બનાવ્યો છે.",
...
class ChatbotGUI:
    def __init__(self, master):
        ...
        self.master.configure(bg="#E6E6FA")  # Lavender background

        self.current_lang = 'en'  # Default language
        self.state = "initial"  # Chatbot state machine for quiz and other multi-step commands
        self.quiz_score = 0
        self.quiz_question_num = 0

        self.language_frame = tk.Frame(self.master, bg="#E6E6FA", padx=20, pady=20)
        self.language_frame.pack(fill=tk.BOTH, expand=True)

        lang_label = tk.Label(self.language_frame, text=LANG_DATA[self.current_lang]['lang_select_prompt'], font=("Helvetica", 16, "bold"), bg="#E6E6FA")
        lang_label.pack(pady=(50, 20))

        # Language selection buttons
        self.lang_buttons = {}
        for lang_code, data in LANG_DATA.items():
            btn = tk.Button(self.language_frame, text=data['lang_desc'], font=("Helvetica", 14),
            ...
            btn.pack(pady=10, fill=tk.X, padx=100)
    ...
    def create_widgets(self):
        ...
        self.chat_display = scrolledtext.ScrolledText(self.master, wrap=tk.WORD, bg="#F0F8FF", fg="#333333", font=chat_font, bd=0, padx=10, pady=10)
        self.chat_display.pack(padx=20, pady=(20, 10), fill=tk.BOTH, expand=True)
        self.chat_display.config(state=tk.DISABLED)

        # Input field and send button frame
        input_frame = tk.Frame(self.master, bg="#E6E6FA")
        input_frame.pack(padx=20, pady=10, fill=tk.X)

        self.user_input = tk.Entry(input_frame, font=("Helvetica", 12), bd=0, relief=tk.GROOVE, bg="white", fg="#333333")
        self.user_input.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=5)
        self.user_input.bind("<Return>", self.send_message_event)

        self.send_button = tk.Button(input_frame, text="Send", font=("Helvetica", 12, "bold"), bg="#4682B4", fg="white", bd=0, padx=15, pady=5, activebackground="#5F9EA0", activeforeground="white", command=self.send_message)
    ...
    def process_command(self, command):
        ...
        if command == 'info':
            ...
            if month in [3, 4, 5, 6, 7, 8]:  # Summer/Monsoon
                if 6 <= hour < 18:
                    weather_desc = "Hot and humid with a chance of rain."
                else:
                    weather_desc = "Warm and clear."
            else:  # Winter
                if 6 <= hour < 18:
                    weather_desc = "Cool and pleasant."
                else:
                    weather_desc = "Cold with a clear sky."
    ...
    def handle_image_generation(self, prompt):
        ...
        # This is a dummy URL. In a real application, you would use an API.
        image_url = "https://example.com/generated_image/" + prompt.replace(" ", "_")
